# Expert Listing – Backend Service

Express API that serves the expert directory for the Expert Listing platform.
...
| GET | `/api/experts?specialty=<name>` | List experts, optionally filtered |
| GET | `/api/experts/:id` | Get one expert |
...
docker run --rm -p 3000:3000 backend:local
...
```
ghcr.io/experts-listing/backend:<git-sha>
<account-id>.dkr.ecr.<region>.amazonaws.com/expert-listing/backend:<git-sha>
```
...
| Variable | `AWS_ECR_PUSH_ROLE_ARN` | IAM role assumed via OIDC. It can only push to `expert-listing/backend` (output of Infra-Service Terraform) |
